...
def generate_meal_plan(target_plan: MealPlan, session: Session):
    ...
    if "Chicken Breast" in food_map:
        items.append(
            MealPlanItem(
                meal_plan_id=target_plan.id,
                food_item_id=food_map["Chicken Breast"].id,
                amount=1.5,
    ...
    if "White Rice (Cooked)" in food_map:
        items.append(
            MealPlanItem(
                meal_plan_id=target_plan.id,
                food_item_id=food_map["White Rice (Cooked)"].id,
                amount=1.5,
                meal_type=MealType.LUNCH,
            )
        )
    if "Broccoli" in food_map:
        items.append(
            MealPlanItem(
                meal_plan_id=target_plan.id,
                food_item_id=food_map["Broccoli"].id,
                amount=1.0,
                meal_type=MealType.LUNCH,
            )
        )
    ...
    if "Greek Yogurt" in food_map:
        items.append(
            MealPlanItem(
                meal_plan_id=target_plan.id,
                food_item_id=food_map["Greek Yogurt"].id,
                amount=1.5,
